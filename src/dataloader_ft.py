...
class AudiosetDataset(Dataset):
	def __init__(self, dataset_json_file, audio_conf, label_csv=None):
		...
		if self.dataset == 'vggsound':
			...
			if self.audio_conf['mode'] == 'train':
				con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/train_vgg.sqlite.db' + "?mode=ro", uri=True)
				self.cur = con.cursor()
				self.num_samples = self.cur.execute("SELECT COUNT(*) FROM annos").fetchone()[0]
			else:
				con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/val_vgg.sqlite.db' + "?mode=ro", uri=True)
				# con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/val_vgg_retrieval.sqlite.db' + "?mode=ro", uri=True) ### <----- YB: for retrieval
				self.cur = con.cursor()
				self.num_samples = self.cur.execute("SELECT COUNT(*) FROM annos").fetchone()[0]

		elif self.dataset == 'audioset_20k':
			if self.audio_conf['mode'] == 'train':
				con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/train_as_20k.sqlite.db' + "?mode=ro", uri=True)
				self.cur = con.cursor()
				self.num_samples = self.cur.execute("SELECT COUNT(*) FROM annos").fetchone()[0]
			else:
				con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/eval_as_v2.sqlite.db' + "?mode=ro", uri=True)
				# con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/val_as_retrieval.sqlite.db' + "?mode=ro", uri=True)
				self.cur = con.cursor()
				self.num_samples = self.cur.execute("SELECT COUNT(*) FROM annos").fetchone()[0]
			...
			if self.audio_conf['mode'] == 'train':
				con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/train_as_2m_v2.sqlite.db' + "?mode=ro", uri=True)
				self.cur = con.cursor()
				self.num_samples = self.cur.execute("SELECT COUNT(*) FROM annos").fetchone()[0]
			else:
				con = sqlite3.connect("file:" + '/mnt/opr/yblin/cav-pt/eval_as_v2.sqlite.db' + "?mode=ro", uri=True)
	...
	def __getitem__(self, index):
		...
		if self.dataset == 'vggsound':
			video_path = '/mnt/opr/yblin/vggsound/'+datum[1]+'.mp4'
			...
			video_path_mix = '/mnt/opr/yblin/vggsound/'+mix_datum[1]+'.mp4'
			...
			if self.audio_conf['mode'] == 'train':
				...
				video_path_mix = '/mnt/opr/yblin/audioset_sun/train_balanced/'+mix_datum[1]+'.mp4'
			else:
				video_path = '/mnt/opr/yblin/audioset_sun/eval_segments/'+datum[1]+'.mp4'
				# video_path = '/data/yanbo/download_tool/as_retrieval/'+datum[1]+'.mp4'
				...
				video_path_mix = '/mnt/opr/yblin/audioset_sun/eval_segments/'+mix_datum[1]+'.mp4'

		elif self.dataset == 'audioset_2m':

			if self.audio_conf['mode'] == 'train':
				video_path = '/mnt/opr/yblin/audioset_sun/train_2m/'+datum[1]+'.mp4'
				mix_sample_idx = random.randint(0, self.num_samples-1)
				query = f"SELECT * FROM annos WHERE id = {mix_sample_idx};"
				res = self.cur.execute(query)
				mix_datum =  res.fetchone()

				video_path_mix = '/mnt/opr/yblin/audioset_sun/train_2m/'+mix_datum[1]+'.mp4'
			else:
				video_path = '/mnt/opr/yblin/audioset_sun/eval_segments/'+datum[1]+'.mp4'
				mix_sample_idx = random.randint(0, self.num_samples-1)
				query = f"SELECT * FROM annos WHERE id = {mix_sample_idx};"
