...
class AudiosetDataset(Dataset):
	def __init__(self, dataset_json_file, audio_conf, label_csv=None):
		...
		# self.data = self.yb_check_file_extis(self.data)

		if self.audio_conf['mode'] == 'train':
			# self.data = np.load('/data/yanbo/uavm/egs/audioset/train_new.npy')

			# # ###------> VGGS
			# self.vgg_data = np.load('/mount/opr/yblin/vggsound_train.npy')
			# self.data = np.concatenate((self.data, self.vgg_data), axis=0)
			self.data = np.load('/mnt/opr/yblin/audioset_sun/train_20k.npy')
		else:
			self.data = np.load('/mnt/opr/yblin/audioset_sun/eval.npy')
		self.num_samples = self.data.shape[0]
		...
		# self.num_samples = self.data.shape[0]
	# 1772023
	def yb_check_file_extis(self, data_np):
		
		# if self.audio_conf['mode'] == 'train':
		# 	find_out = glob("/data/yanbo/Dataset/audioset-processing/output/train_2m/*.mp4")
		# 	# 1736998
		# else:
		# 	find_out = glob("/data/yanbo/Dataset/audioset-processing/output/eval_segments/*.mp4")
		my_total = []
		for idx, item in enumerate(data_np): 
			print('########### {}'.format(idx))
			my_path = '/data/yanbo/Dataset/audioset-processing/output/train_2m/' + item[0].split('/')[-1].replace('.flac','.mp4')
			if os.path.exists(my_path):
				my_total.append(idx)
		# data_np = np.array(find_out, dtype=str)
		np.save('/data/yanbo/uavm/egs/audioset/train_new.npy',data_np[my_total])

		np.savetxt("/data/yanbo/uavm/egs/audioset/datafile/audioset_2m_cleaned_weight_yb.csv", self.samples_weight[my_total], delimiter=",")
	...
	# def yb_parse_name(self, filename): ## yb:!!!!!!
	# 	return filename
	# 	tmp = os.path.splitext(filename)[0]
	# 	new_path = '/playpen-storage/yblin/low/'+tmp.split('/')[-1]+'.mp4'
	...
	def __getitem__(self, index):
		if random.random() < self.mixup:
			...
			try:
				...
				# vr.seek(0)
				# image = vr.get_batch(np.linspace(0, len(vr)-1 , num=10, dtype=int))
				
				
				# image = image/255
				# # image = torch.from_numpy(image)
				# image = self.my_normalize(image.permute(0,3,1,2)) #
