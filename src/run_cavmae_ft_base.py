...
parser.add_argument('-b', '--batch_size', default=48, type=int, metavar='N', help='mini-batch size')
...
parser.add_argument("--ftmode_test", type=str, default=None, help="how to fine-tune the model")
...
parser.add_argument("--mm_lr", type=float, default=None, help="learning rate ratio the newly initialized layers / pretrained weights")
...
parser.add_argument("--dis_w", type=float, default=0, help="weight term for distillation loss")
parser.add_argument("--dis_w_2", type=float, default=0, help="weight term for distillation loss")
...
if args.bal == 'bal':
    ...
    if args.weight_file == None:
        samples_weight = np.loadtxt(args.data_train[:-5]+'_weight_v2.csv', delimiter=',')
    else:
        samples_weight = np.loadtxt(args.data_train[:-5] + '_' + args.weight_file + '.csv', delimiter=',')

    sampler = WeightedRandomSampler(samples_weight, len(samples_weight), replacement=True)

    # yb_sampler = DistributedProxySampler(sampler)
    # train_loader = torch.utils.data.DataLoader(dataset=dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf), 
    # batch_size=args.batch_size, num_workers=args.num_workers, sampler=yb_sampler, pin_memory=True, persistent_workers=False)


    train_loader = torch.utils.data.DataLoader(dataset=dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf), 
    batch_size=args.batch_size, num_workers=args.num_workers, sampler=sampler, pin_memory=True, persistent_workers=False)
    ##### <----------
else:
    ...
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf),
        batch_size=args.batch_size, sampler=train_sampler, shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=True)
...
if args.model == 'cav-mae-ft':
    ...
    audio_model = models.CAVMAEFT_BASE(label_dim=args.n_class, modality_specific_depth=9999)
...
if args.skip_frame_agg == True:
    ...
    for frame in range(total_frames):
        ...
        val_loader = torch.utils.data.DataLoader(
            dataloader.AudiosetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf),
            batch_size=args.batch_size, shuffle=False, sampler=val_sampler, num_workers=args.num_workers, pin_memory=True)
        

        stats, audio_output, target = validate(audio_model, val_loader, val_sampler, args, output_pred=True)
